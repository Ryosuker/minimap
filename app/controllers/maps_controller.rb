class MapsController < ApplicationController
    ...
    respond_to do |format|
      ...
      format.json do
        render json: {
          ...
          coworks: @coworks.as_json(include: :work_place_images),      #⭐️あとで修正箇所
  ...
  def search_places(latitude, longitude)
    ...
    @coworks = circle_search(Cowork, latitude, longitude)        #⭐️あとで修正箇所
  ...
  def circle_search(model, latitude, longitude)
    ...
    model.includes(:work_place_images).within(1, origin: [latitude, longitude]).by_distance(origin: [latitude, longitude])
  end
end